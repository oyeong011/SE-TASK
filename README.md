...
# 6.9 자동 링크(확장)
GFM을 사용하면 더 많은 조건에서 자동 링크가 인식되는 확장을 지원합니다.autolink

Autolinks는 더 작은 상황에서 인식되지만 사용을 요구하지 않고 이를 구분할 수 있습니다. 이러한 인식된 모든 자동 링크는 줄의 시작 부분, 공백 후 또는 구분문자, 및 .<>*_~(

텍스트가 발견된 다음 유효한 도메인이있을 때 확장된 www 자동 링크가 인식됩니다. 유효한 도메인은 유수 문자 세그먼트, 밑줄() 및 하이픈()이 기간()으로 구분된 세그먼트로 구성됩니다. 하나 이상의 기간이 있어야 하며 도메인의 마지막 두 세그먼트에는 밑줄이 없을 수 있습니다.www._-.

구성표가 자동으로 삽입됩니다.http
...
Visit www.commonmark.org/help for more information.
### EX 3
그런 다음 확장 자동 링크 경로 유효성 검사를 다음과 같이 적용합니다.

후행 문장 부호 (특히,,, , , , , , 및) 그들은 링크의 내부에 포함 될 수 있지만, 자동 링크의 일부로 간주되지 않습니다 :?!.,:*_~
```txt
...
```
### EX 4
자동 링크가 끝나면 전체 자동 링크를 검사하여 총 괄호 수를 검사합니다. 닫는 괄호가 개괄호를 여는 것보다 많은 수의 경우 괄호 안에 자동 링크를 포함하기 위해 자동 링크의 타의 추종을 불허하는 괄호 부분을 고려하지 않습니다.)
```txt
...
```
### EX 8
확장된 URL 자동 링크는 스키마 중 하나 또는 유효한 도메인다음에 확장된 자동 링크 경로 유효성 검사에따라 0 개 이상의 비 공간 비 문자가 있을 때 인식됩니다.http://https://<
```txt
http://commonmark.org
...
한 광석은 거형, 또는 , 또는 ..-_+
기호입니다.@
한 개 이상의 문자는 유수또는 마침표()로 구분됩니다. 적어도 하나의 기간이 있어야합니다. 마지막 문자는 하나이거나 .-_.-_
...
```
### EX 10
+ 그 전에 는 발생할 수 있지만 그 후에는 발생하지 않습니다.@
```txt
...
# 6.11 허용되지 않는 원시 HTML(확장)
GFM을 사용하면 HTML 출력을 렌더링할 때 다음 HTML 태그가 필터링되는 확장을 사용할 수 있습니다.tagfilter
